...
class CadastrarEmpresa(forms.ModelForm):
    ...
    cnpj = BRCNPJField(max_length=18, min_length=14, widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'cnpj'}))
...
class CadastrarOrdem(forms.ModelForm):
    cliente = forms.ModelChoiceField(queryset=Cliente.objects.all(), widget=forms.Select(attrs={'class': 'cliente'}))
    data = forms.DateField()
    num_nf_entrada = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'num_nf_entrada'}))
    descricao = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'descricao'}))
    acessorios = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'acessorios'}))
    observacoes = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'observacoes'}))
    ...
    data_liberacao = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'liberacao'}))
    num_nf_servico = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'data_nf_servico'}))
    num_nf_vendas = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'num_nf_vendas'}))
    num_nf_devolucao = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'num_nf_devolucao'}))
    responsavel_preenchimento = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'responsavel_preenchimento'}))

    class Meta:
        model = Ordem
        exclude = ('data',)
        fields = ('cliente', 'num_nf_entrada', 'descricao', 'acessorios', 'observacoes', 'data_envio', 'data_aprovado',
